...
HelpLibraryManagerLauncher.exe /product "VS" /version "100" /locale en-us /uninstall /silent /vendor "AutomationML eV" /productName "AutomationMLEditor" /mediaBookList "The AutomationML Editor PlugIn Implementation Guide" > NUL
...
REM build time.
IF EXIST "AMLEditorPluginDocumentation.msha" COPY /Y "AMLEditorPluginDocumentation.msha" HelpContentSetup.msha
...
REM Uninstall first in case it is already there.  If not, it won't install below.  We'll ignore any error output
REM by redirecting it to NUL.
HelpLibraryManagerLauncher.exe /viewerVersion %1  /locale en-us /wait 0 /operation uninstall /vendor "AutomationML eV" /productName "AutomationMLEditor" /bookList "The AutomationML Editor PlugIn Implementation Guide" > NUL

REM Install the new content.
HelpLibraryManagerLauncher.exe /viewerVersion %1  /locale en-us /wait 0 /operation install /sourceUri "%CD%\AMLEditorPluginDocumentation.msha"
